...
export default function HeaderClient({ headerData }) {
  ...
  const buttonClasses = "px-4 py-2 text-white bg-black border border-black rounded-md hover:bg-gray-800 transition-colors text-sm font-semibold";
  
  // Style to hide elements when the menu is open on mobile
  const hideOnMobileOpen = isMenuOpen ? 'invisible md:visible' : '';

  return (
    <Bounded full className={`absolute top-3 z-50 w-full ${isMenuOpen ? 'fixed top-0 bg-transparent' : ''}`}>
      <header className="flex flex-row items-center justify-between md:px-6 py-4">
        ...
        <div className={`md:block ${hideOnMobileOpen} ${logoColor}`}>
          <TerratoneLogo className='scale-70 sm:scale-75 lg:scale-90'/>
        </div>
